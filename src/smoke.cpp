...
void SMOKE::LoadEngine(const std::string& engine_path) {
    ...
    engine_ = runtime_->deserializeCudaEngine(trt_model_stream.get(), length);
...
{   
    ...
    engine_ = runtime_->deserializeCudaEngine(plan_->data(), plan_->size());


    //save engine file
    size_t pos = onnx_path.find("/model/");
    std::string model_loc = onnx_path.substr(0, pos);
    std::string engine_path = model_loc + "/model/smoke_dla34.engine";
...
void SMOKE::PostProcess(cv::Mat& input_img) 
{
    ...
    for (int i = 0; i < TOPK; ++i) 
    {
        ...
        Bbox detector_box;
        detector_box.class_id = class_id;
        detector_box.angle = angle;
        detector_box.length = l;
        detector_box.width = w;
        detector_box.height = h;
        detector_box.x = x;
        detector_box.y = y;
        detector_box.z = z;
        detector_result.push_back(detector_box);
    }
